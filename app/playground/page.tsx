...
export default function PlaygroundPage(){
  ...
  const setBlocks=(fn:(b:PageVersionContent)=>PageVersionContent)=>setBlocksState(prev=>fn(prev));

  // Accept a constructed block from the library
  const addFromLibrary = (raw: any) => {
    const id = raw.id ?? crypto.randomUUID();
    const baseW = raw.layout?.w ?? 400;
    const baseH = raw.layout?.h ?? 240;
    // center on design canvas; user can move after
    const block: Block = {
      id,
      type: raw.type,
      props: raw.props ?? {},
  ...
  async function saveVersion(name:string, folderId?:string){
    ...
    const { data: pageRow, error: selErr } = await s.from('pages').select('id').eq('slug','home').maybeSingle();
    if (selErr && selErr.code !== 'PGRST116') { alert(`Load page failed: ${selErr.message}`); return; }
    let pageId = pageRow?.id as string | undefined;
    if (!pageId){
      const { data: created, error: insErr } =
        await s.from('pages').insert({ slug:'home', title:'Home', folder_id: folderId ?? null }).select('id').single();
      if (insErr || !created?.id){ alert(`Create page failed: ${insErr?.message ?? 'no id returned'}`); return; }
      pageId = created.id;
    }
    const { error: verErr } = await s.from('page_versions').insert({
      page_id: pageId, name, content: blocks, is_playground:true, design_width:1440, design_height:900
    });
