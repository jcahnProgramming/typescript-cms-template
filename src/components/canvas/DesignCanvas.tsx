...
export default function DesignCanvas({
  ...
  const onClickCanvas=()=>{ onSelect(null); if(menu.open) setMenu(m=>({...m,open:false})); };
  ...
  const addAtCursor=(factory:()=>any)=>{
    ...
    const layout = { x, y, w: raw?.layout?.w ?? 400, h: raw?.layout?.h ?? 240, z:1 };
        ...
        {menu.open && rootRef.current && (
          <div
            style={{
              ...
              borderRadius:8, padding:8, width:220, maxHeight:300, overflow:'auto'
            }}
          >
            {palette.map((w)=>(
              <div
